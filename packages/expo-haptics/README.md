...
`yarn add expo-haptics` or `npm install expo-haptics`
...
2.  Insert the following lines inside the dependencies block in `android/app/build.gradle`:
    ```gradle
    compile project(':expo-haptics')
    ```
3.  Add `new ExpoHapticsPackage()` to your module registry provider in `MainApplication.java`.
...
export default class App extends Component {
  ...
  doGreatJob = async () => {
    await ExpoHaptics.someGreatMethod();
  }

  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.paragraph}>'expo-haptics' example</Text>
      </View>
    );
  }
}
...
### `ExpoHaptics.someGreatMethod(options)`
